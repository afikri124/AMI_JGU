...
<span class="text-muted fw-light">Audit Plan / Add /</span>
@endsection
@section('title', 'Determine Standard')
...
@section('content')
<div class="card">
    <form action="{{ route('create_auditor_std', $data->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="card-header">
            <h3><b>Determine Standard</b></h3>
            <hr class="my-0">
        </div>
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-lg-6 col-md-12 mb-3">
                    ...
                    <div class="form-group">
                        <label for="standard_category_id_{{ $auditor->id }}" class="form-label"><b>Category</b><i
                                class="text-danger">*</i></label>
                        <select name="standard_category_id[{{ $auditor->id }}][]"
                            id="standard_category_id_{{ $auditor->id }}"
                            class="form-select select2 standard_category_id" multiple required>
                            @foreach($category as $c)
                            <option value="{{ $c->id }}"
                                {{ in_array($c->id, $selectedCategories[$auditor->id] ?? []) ? 'selected' : '' }}>
                                {{ $c->id }} - {{ $c->description }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <p></p>
                    <div class="form-group">
                        <label for="standard_criteria_id_{{ $auditor->id }}" class="form-label"><b>Criteria</b><i
                                class="text-danger">*</i></label>
                        <select name="standard_criteria_id[{{ $auditor->id }}][]"
                            id="standard_criteria_id_{{ $auditor->id }}" class="form-select select2" multiple required>
                            @foreach($criteria as $c)
                            <option value="{{ $c->id }}"
                                {{ in_array($c->id, $selectedCriteria[$auditor->id] ?? []) ? 'selected' : '' }}>
                                {{ $c->id }} - {{ $c->title }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    success: function (results) {
                        ...
                        $.each(results, function (key, value) {
                            if (selectedCategories.includes(value
                                    .standard_category_id.toString())) {
                                criteriaOptions += '<option value="' + value.id +
                                    '">' + value.title + '</option>';
                            }
                        });
                        ...
                        $('#standard_criteria_id_' + auditorId).val(selectedCriteria[
                            auditorId] || []).trigger('change');
                    }
                });
            } else {
                $('#standard_criteria_id_' + auditorId).html(
                    '<option value="">Select Standard Criteria</option>').trigger('change');
            }
        });
